...
const TargetCursor: React.FC<TargetCursorProps> = ({
  targetSelector = 'button, a, input, select, textarea, [role="button"], .cursor-target', // Updated default to catch common elements
  spinDuration = 2,
  ...
  useEffect(() => {
    ...
    // Using a broad target selector to catch standard interactive elements
    const handlePotentialTarget = (e: MouseEvent) => {
        if (!cursorRef.current) return;
        ...
        if (target && target !== activeTarget) {
             // Force a leave on the previous target if it exists
             if (activeTarget) {
                 // Trigger the cleanup/leave logic manually here if needed, 
                 // but typically mouseover on new element implies mouseleave on old
                 // unless they are nested.
             }
             // Proceed to enter logic (which is the original enterHandler)
             // We can just rely on the original event listener approach, 
             // provided 'targetSelector' is broad enough.
        }
    };
    ...
    const enterHandler = (e: MouseEvent) => {
      const directTarget = e.target as Element;
      // Use .closest to find if we are inside a target (e.g. icon inside button)
      const target = directTarget.closest(targetSelector);
      ...
      if (activeTarget) {
        // If we moved directly from one target to another without a mouseleave firing
